  ...
  return <div className="bg-[#071a17] text-[#fffaf0]">
      ...
      <div className="page-wrap relative z-10 grid min-h-[calc(100vh-72px)] items-end gap-12 pb-10 pt-20 lg:grid-cols-[1.2fr_.8fr] lg:pb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8 }}>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Link href="/onboarding" className="inline-flex items-center gap-3 bg-[#e88a34] px-6 py-4 text-xs font-bold uppercase tracking-[.14em] text-[#10211d]">Experience the demo <ArrowRight className="h-4 w-4" /></Link>
            <Link href="/dashboard" className="inline-flex items-center gap-3 border border-white/18 px-6 py-4 text-xs font-bold uppercase tracking-[.14em] text-white/72">Open financial home</Link>
          </div>
        </motion.div>
        <motion.div className="relative self-center lg:justify-self-end" initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .25, duration: .9 }}>
          <div className="relative grid h-[330px] w-[330px] place-items-center sm:h-[410px] sm:w-[410px]">
            ...
            <span className="absolute right-2 top-1/2 flex items-center gap-2 text-[9px] uppercase tracking-widest text-[#e88a34]"><span className="h-2 w-2 rounded-full bg-[#e88a34] breathe" />Live state</span>
          </div>
        </motion.div>
          ...
          <div className="surface overflow-hidden">
            ...
            <div className="grid md:grid-cols-2">
              <div className="p-6 md:p-8">
                ...
                {[['Income','₹82,000'],['Existing EMI','₹31,500'],['Current burden','38.4%'],['Buffer','2.4 months'],['Cash flow','Declining']].map(([a,b]) => <div key={a} className="flex items-center justify-between border-b hairline py-3 text-sm"><span className="text-muted-foreground">{a}</span><span className="font-financial">{b}</span></div>)}
              ...
              <div className="bg-[#102b26] p-6 text-[#fffaf0] md:p-8">
                ...
                <div className="mt-7 space-y-3 text-sm text-white/54"><p>Projected new EMI <b className="float-right font-financial text-white">₹11,447</b></p><p>Projected burden <b className="float-right font-financial text-[#e98a7e]">52.4%</b></p></div>
    ...
    <section className="page-section border-t border-white/10">
      <div className="page-wrap grid gap-12 lg:grid-cols-[1fr_auto] lg:items-end">
        ...
        <Link href="/onboarding" className="inline-flex h-16 items-center gap-4 border border-white/15 px-6 text-xs font-bold uppercase tracking-[.14em] transition hover:bg-white/[.05]">Begin the Ravi journey <ArrowRight className="h-4 w-4 text-[#e88a34]" /></Link>
      </div>
