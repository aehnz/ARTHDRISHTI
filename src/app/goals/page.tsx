...
export default function GoalsPage() {
  ...
  const [goals,setGoals]=useState<Goal[]>(demoState.goals);
  useEffect(()=>{let active=true;const timer=window.setTimeout(()=>api.simulateGoals(extra).then(result=>{if(active)setGoals(result)}).catch(()=>undefined),120);return()=>{active=false;window.clearTimeout(timer)}},[extra]);
  return <div>
    ...
    <section className="page-wrap grid gap-6 py-8 lg:grid-cols-[1.25fr_.75fr]">
      <div className="space-y-5">{goals.map((goal,index)=>{const progress=goal.progress??0; return <article key={goal.id} className="surface overflow-hidden"><div className="grid gap-6 p-6 md:grid-cols-[1fr_auto] md:p-8"><div><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center border border-[#e88a34]/30 bg-[#e88a34]/8">{index===0?(goal.icon==='home'?<Home className="h-4 w-4"/>:<Shield className="h-4 w-4"/>):<Target className="h-4 w-4"/>}</span><div><p className="micro-label">Goal {String(index+1).padStart(2,'0')}</p><h2 className="mt-2 text-2xl font-medium tracking-[-.04em]">{goal.name}</h2></div></div><p className="font-financial mt-8 text-4xl">{formatINR(goal.current)} <span className="text-base text-muted-foreground">of {formatINR(goal.target)}</span></p></div><div className="text-left md:text-right"><p className="micro-label">Projected</p><p className="mt-3 text-lg font-semibold">{goal.projectedCompletion??goal.targetDate}</p><p className="mt-2 text-xs text-muted-foreground">{formatINR(goal.monthlyContribution)}/month</p></div></div><div className="h-2 bg-secondary"><motion.div className="h-full bg-[#2d7a65]" initial={{width:0}} whileInView={{width:`${progress}%`}} viewport={{once:true}}/></div><div className="grid grid-cols-3 gap-px bg-border border-t border-border">{[['Progress',`${progress}%`],['With extra',`${goal.monthsToCompletion??0} months`],['Time saved',`${goal.monthsSaved??0} months`]].map(([a,b])=><div key={a} className="bg-card p-4"><p className="micro-label">{a}</p><p className="font-financial mt-3 text-lg">{b}</p></div>)}</div></article>})}</div>
