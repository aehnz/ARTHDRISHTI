...
export default function AskPage(){
  ...
  const send=async(text=input)=>{if(!text.trim()||loading)return; const question=text.trim(); addChatMessage({role:'user',content:question});setInput('');setLoading(true); try { const response=await api.ask(question,language);addChatMessage({role:'assistant',content:response.message}); } catch (error) { addChatMessage({role:'assistant',content:error instanceof Error?error.message:'ARTHDRISHTI intelligence service is temporarily unavailable.'}); } finally { setLoading(false); }};
  return <div>
    ...
    <section className="page-wrap grid gap-6 py-8 lg:grid-cols-[.72fr_1.28fr]">
      <aside className="surface-dark grid-rule h-fit p-7 lg:sticky lg:top-24"><BrainCircuit className="h-5 w-5 text-[#e88a34]"/><p className="eyebrow mt-7 !text-[#e88a34]">Live context</p><h2 className="mt-4 text-2xl font-medium">{demoState.customer.name}</h2><div className="mt-7 space-y-4 border-t border-white/10 pt-5">{[['Health',`${demoState.financialState.healthScore}/100`],['EMI burden',`${demoState.financialState.debt.emiBurdenRatio}%`],['Buffer',`${demoState.financialState.savings.bufferMonths} months`],['Cash flow',demoState.financialState.cashFlow.trend]].map(([a,b])=><div key={a} className="flex justify-between text-xs"><span className="text-white/40">{a}</span><b className="font-financial capitalize">{b}</b></div>)}</div><div className="mt-8 flex gap-3 border-t border-white/10 pt-6"><ShieldCheck className="h-4 w-4 shrink-0 text-[#75bca5]"/><p className="text-[11px] leading-5 text-white/38">Financial and governance controls remain authoritative. The assistant communicates their governed output.</p></div></aside>
